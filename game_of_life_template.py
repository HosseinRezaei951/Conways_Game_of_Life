...
class Board(object):
    ...
    def seed(self, block_coords):
        ...
        for coordinates in block_coords:
            self.block_list[coordinates].set_live(self.canvas)
        ...
        for col in range(x - 1, x + 2):
            for row in range(y - 1, y + 2):
                if 0 <= col < self.width and 0 <= row < self.height:
                    neighborsList.append(self.block_list[col, row])
        neighborsList.remove(self.block_list[x,y])
    ...
    def count_live_neighbors(self, block):
        '''
        Given a Block object, returns a number of neighbors with live status.
        '''
        count = 0
        neighborsList = self.get_block_neighbors(block)
